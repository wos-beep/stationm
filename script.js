...
function copySummaryText() {
    ...
    userState.selectedIds.forEach(id => {
        ...
        if(diff <= 0 || diff <= 24 * 3600000) {
            ...
            txt += `${s.typeName} Lv.${s.lv}: ${diff <= 0 ? "争奪中" : Math.floor(diff/3600000)+"h残り"}\n`;
        }
    });
...
function updateLvFilters() {
    ...
    if(type) {
        [...new Set(MASTER_DATA[type].coords.map(c => c.lv))].sort((a,b)=>a-b).forEach(l => lvSel.innerHTML += `<option value="${l}">Lv.${l}</option>`);
    }
    renderModalList();
}
function addStation(id) { userState.selectedIds.push(id); userState.modes[id] = 'none'; save(); render(); renderModalList(); }
function setMode(id) { const m = userState.modes[id]; userState.modes[id] = m === 'self' ? 'other' : (m === 'other' ? 'none' : 'self'); save(); render(); }
...
function tick() { const sorted = [...userState.selectedIds].sort((a,b) => ((userState.timers[a] || 0) + DUR - Date.now()) - ((userState.timers[b] || 0) + DUR - Date.now())); render(sorted); }
init();
